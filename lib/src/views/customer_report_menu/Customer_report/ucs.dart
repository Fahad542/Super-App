...
final  String  countmonth;
  ...
  String? year;
  String? month;
  Uc({Key? key, this.dsf_id, this.code, this.name, this.year, this.month, required this.countmonth}) : super(key: key);
  @override
  State<Uc> createState() => _fmsalesState();
}
dsf heading = dsf(
    companyName: "Code" ,companyid: 'Customer Name '
);
List<dsf> dataa = [];
total_uc_report? datalist;
...
class _fmsalesState extends State<Uc> {
  Future<void> getSupervisorReportData(String a, String b, String c) async {
    setState(() {
      isLoading = true;
    });
    var response = await ApiService().uc_details(
...
setState(() {
  ...
  DateTime currentDate = DateTime.now();
  a = currentDate.month.toString().padLeft(2, '0');
  a=widget.countmonth;
});


    getSupervisorReportData(a.toString(),widget.year.toString(),widget.dsf_id.toString());

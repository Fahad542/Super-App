...
import 'package:premier/src/views/customer_report_menu/supervisor_report_screen.dart';
...
class fse extends StatefulWidget {
  ...
  const fse({Key? key, required this.month, required this.year, required this.id, required this.name, required this.month_number}) : super(key: key);

  @override
  State<fse> createState() => _dsf_company_wiseState();
}

class _dsf_company_wiseState extends State<fse> {
  ...
  hand heading = hand(
      companyName: "ID/"
          "FSE Name", invCount: "Total Cust", totalSales: "UC's"
  ...
  Future<void> getSalesReportData(String a, String b) async {
    ...
    var response = await ApiService().fsecust(context, a, b,widget.id);
    ...
    response?.when(success: (_s) async {
      ...
      _s.data?.forEach((element) {
        dataa.add(hand(
          companyName:
          '${element.fse_id.toString()}-${element.fse_name.toString()}',
  ...
  Widget build(BuildContext context) {
    return Scaffold(  appBar: AppBar(
      ...
      title:
      Text(
        widget.name == '' ? "FSE customer report" : widget.name,
                  ...
                  child: SingleChildScrollView(
                    ...
                    handdetails(
                      ...
                      onTap: (int value, String name,String code) {
                        ...
                        print(name);
                        Navigator.push(
                            context, MaterialPageRoute(builder: (context) => supervisor_screen( month: selectedMonth, year: selectedyear.toString(),
                          month_number: index.toString(),fm_id: value.toString(),fm_name: name

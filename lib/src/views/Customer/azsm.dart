...
class azsm_sale extends StatefulWidget {
  ...
  const azsm_sale({Key? key, required this.id, required this.name, required this.startdate, required this.enddate, required this.check}) : super(key: key);

  @override
  State<azsm_sale> createState() => _dsf_company_wiseState();
}

class _dsf_company_wiseState extends State<azsm_sale> {
  ...
  hand heading = hand(
      companyName: "ID/"
          "AZSM Name", invCount: "Total Inv", totalSales: "Sale Amt");
  List<hand> dataa = [];
  Future<void> getSalesReportData(String a, String b) async {
    setState(() {
      isLoading = true;
    });
    var response = await ApiService().azsmsalereport(context, a, b,widget.id);
    ...
    response?.when(success: (_s) async {
      ...
      _s.data?.forEach((element) {
        dataa.add(hand(
          companyName: '${element.azsm_id.toString()}-${element.azsm_name.toString()}',
  ...
  Widget build(BuildContext context) {
    return Scaffold(  appBar: AppBar(
      ...
      title: Text(
        widget.name.isEmpty ?"AZSM Sale Report" :widget.name,
                  ...
                  child: SingleChildScrollView(
                    ...
                    handdetails(
                      ...
                      onTap: (int companyid, String name, String amount) {
                        ;
                        Navigator.push(context, MaterialPageRoute(builder: (context) => fmcompanysales(zsm_id: companyid.toString(), name: name, startdate: formattedStartDate, enddate: formattedEndDate, check: widget.check,)));
                      },
                    ),
                  ),
                ),
